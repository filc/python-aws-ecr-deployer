...
def compare_image_versions(ecs_images, ecr_images):
    ...
    def _extend_result_with_only_ecs_images(compare_result):
        compare_result.update({
            ecs_image: _get_result('', _get_ecs_tag_from_services(ecs_info)[0], ImageStatuses.ONLY_IN_ECS)
    ...
    def _get_ecs_tag_from_services(services):
        ...
        if len(set(ecs_tags)) == 1:
            ...
            ecs_tag = ','.join(['::'.join(s) for s in services])
            single_version = False

        return ecs_tag, single_version

    compare_result = {}

    for ecr_image, ecr_info in ecr_images.items():
        ecr_tag = ecr_info[0]
        ecs_tag, single_ecs_version = _get_ecs_tag_from_services(ecs_images.get(ecr_image, [['']]))

        if not single_ecs_version:
            compare_result[ecr_image] = _get_result(ecr_tag, ecs_tag, ImageStatuses.MORE_VERSIONS_IN_ECS)
        else:
